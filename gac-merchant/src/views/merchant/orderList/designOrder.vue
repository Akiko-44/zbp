...
<div class="app-container calendar-list-container">
	<el-form :inline="true" :model="listQuery" :rules="rules" ref="designListQueryForm" label-width="101px" class="listQueryForm">
  	<div class="filter-container">
		  ...
		  <el-form-item label="订单状态:">
		    <el-select v-model="listQuery.status" @change="handleFilter" clearable placeholder="订单状态" style="width: 120px;">
		      <el-option
		  ...
		  <el-form-item label="是否使用优惠:">
				<el-select v-model="listQuery.isCoupons" @change="handleFilter" placeholder="是否使用优惠" style="width: 150px;">
		      <el-option
  ...
  <div class="filter-container">
	  ...
	  <el-form-item label="订单类型:">
	    <el-select v-model="listQuery.caseType" @change="handleFilter" placeholder="订单类型" style="width: 120px;">
	      <el-option
  ...
  <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
    ...
    <el-table-column align="center" label="商品名称">
      <template slot-scope="{ row }">
        <span v-for="(item,index) in row.orderGoodVOList" :key="index">
    ...
    <el-table-column fixed="right" align="left" label="操作">
      <template slot-scope="{ row, $index }">
        <el-button
          type="primary"
          size="small"
          @click="$router.push({ name: 'designOrderDetail', query: { id: row.orderNumber } })"
        >
          订单详情
        </el-button>
        <el-button
...
//    this.getList()
      this.$router.replace({ name: 'designOrderList', query: { page: val }})
    },
    resetQuery() {
      this.listQuery = getInitQuery()
      this.payDateValue = ''
      this.$refs['designListQueryForm'].resetFields()
      this.$router.replace({ name: 'designOrderList'})
      this.getList()
    },
    cancelOrder(orderNo) {
    	this.$prompt('请输入取消原因', {
