<template>
  <div class="app-container">
    <el-form
      ...
      label-width="140px"
    >
      <el-form-item label="注册主体">
        <el-radio-group v-model="form.registType">
      ...
      >
        <el-input
          ...
          :maxlength="50"
          :placeholder="form.registType === 2 ? '详细地址，具体到小区、街道，与营业地址保持一致' : '请输入详细地址，具体到小区、街道'"
        ></el-input>
      </el-form-item>
      ...
      >
        <el-input
          ...
          :maxlength="20"
          :placeholder="form.registType === 2 ? '请输入开户许可证上的开户名称，一般与企业名称保持一致' : '请输入开户人的真实姓名，与实名认证者保持一致'"
        ></el-input>
      </el-form-item>

      <el-form-item
        :label="form.registType === 2 ? '开户账号' : '个人银行账号'"
        prop="bankAccount"
      >
        <el-input
      ...
      >
        ...
        <ImageUpload
          v-show="!form.materials"
          :multiple="false"
          prefix="materials"
          @successCBK="materialsUploadSuccess"
        />
        <div>(包括但不限于营业场所租赁协议或者产权证明、集中经营场所管理方出具的证明文件等能够反映小微商户真实、合法从事商品或服务交易活动的材料；1.5兆（M）以内，5张，格式支持jpg\jpeg\png，尺寸不限；)</div>
      </el-form-item> -->
      <el-form-item
        label="辅助材料照片"
        prop="materials"
...
export default {
  ...
  methods: {
    ...
    openDialog() {
      ...
      if (this.form.registType === 2) {
        this.$refs.authenticationDialog.title = '法人实名认证'
      }
    ...
    /* materialsUploadSuccess(imgs) {
      this.form.materials = imgs[imgs.length - 1]
    }, */
    materialsUploadSuccess(imgs) {
      imgs.forEach((key, i) => {
