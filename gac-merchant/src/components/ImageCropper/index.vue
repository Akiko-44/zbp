  ...
  methods: {
    ...
    async upload() {
      let that = this,
        ...
        fmData = new FormData()
      fmData.append('file', data2blob(createImgUrl, mime), field + '.' + imgFormat)
      //图片
      /*const token = await getToken()
      fmData.append('token', token)
      fmData.append('key', getKey(prefix, width, height, imgFormat))
      ...
      axios({
        ...
        that.loading = 2
			//图片
//      that.$emit('crop-upload-success', domain +'/'+ resData.data.key)
        that.$emit('crop-upload-success', resData.data.data.picUrl)
      }).catch(err => {
        console.dir(err)
        if (that.value) {
          that.loading = 3
