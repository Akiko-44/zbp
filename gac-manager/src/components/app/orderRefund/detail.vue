<template>
  <div class="app-container"
   ...
   -->
      ...
      <h4>清单：</h4>
      <el-table :data="form.revoList"
                style="width: 620px">
        <el-table-column width="180px"
                         align="center"
                         label="图片">
          <template slot-scope="{ row }">
            <img :src="row.skuMainPic | setImg"
                 style="height: 100px;" />
          </template>
        </el-table-column>
        ...
        <el-table-column width="110px"
                         ...
                         label="商品规格">
          <template slot-scope="{ row }">
            <p v-for="(item, i) in row.skuExplain"
               :key="i">{{item.specsName}}：{{item.attrValue}}</p>
          </template>
        </el-table-column>
      ...
      <div class="auditAction">
        ...
        <el-button v-if="form.auditState == 2 && form.delayedReceipt == 0"
                   type="primary"
                   size="small"
    ...
    <arbitrate-apply-dialog ref="arbitrateApplyDialog"
