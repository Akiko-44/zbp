<template>
	<div class="app-container" v-loading="loading" style="padding-left: 40px">
		<div class="title">
	   ...
	   <div class="info refund-info">
	   		...
	   		<div class="info-body">
   				...
   				<div class="img-box">
   					<img @click="showBigImg(item)" class="small-img" :src="getImg(item)" v-for="item in picList"/>
   				</div>   				
	   		</div>
	   </div>
	   <div class="info arbitrate-info">
	   		...
	   		<div class="info-body">
   				...
   				<div class="img-box">
   					<img @click="showBigImg(item)" class="small-img" :src="getImg(item)" v-for="item in arbitrationPic"/>
   				</div>   				
	   		</div>
	   </div>
	   <div class="btns">
	   	<el-button @click="goback">返回</el-button>
	   	<el-button @click="audit(1)" v-if="refundType === 1 && arbitrationInfo.auditState === 7">同意退款</el-button>
	   	<el-button @click="audit(2)" v-if="refundType === 1 && arbitrationInfo.auditState === 7">拒绝退款</el-button>
	   	<el-button @click="audit(1)" v-if="refundType === 2 && arbitrationInfo.auditState === 7">同意退货</el-button>
	   	<el-button @click="audit(2)" v-if="refundType === 2 && arbitrationInfo.auditState === 7">拒绝退货</el-button>
	   	<!--<el-button @click="audit(3)" v-if="arbitrationInfo.auditState === 7">退回</el-button>-->
	   </div>
	   <el-dialog title="大图" :visible.sync="imgDialogVisible" width="30%">
		  <img class="big-img" :src="getImg(imgUrl)"/>
		</el-dialog>
	  <arbitrate-dialog ref="auditDialog" @success="goback" />
	</div>
</template>
...
//    form: {},
  ...
  async mounted() {
    this.loading = true
    let form = await this.getDetail(this.id).then(data => data.data)
    setTimeout(() => {
