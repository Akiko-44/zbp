  ...
  <el-form :model="form" ref="form" label-width="0px">
    <el-form-item>
      ...
      >
        ...
        <el-table-column width="180px" align="center" label="商家/个人ID">
          <template slot-scope="{ row }">
            <el-input v-model="row.userid" placeholder="请输入商家/个人ID"></el-input>
          </template>
        </el-table-column>
        <el-table-column width="250px" align="center" label="缩略图">
          <template slot-scope="{ row, $index }">
            ...
            <el-button v-else @click="cropShow = true;width=225;height=300; currentIndex = $index" plain size="small">上传</el-button>
          </template>
        </el-table-column>
      ...
      <el-button icon="el-icon-plus" @click="form.data.push({ userid: '', thumbnail: '' })">添加</el-button>
...
<script>
import { getJewelryTalent, saveJewelryTalent } from '@/api/swap/homeSettings'
...
export default {
  ...
  created() {
    ...
    getJewelryTalent().then(result => {
      this.listLoading = false
      const talentList = []
      result.data.records.forEach(function(v, i) {
        talentList.push(
          {
            userid: v.userId,
  ...
  methods: {
    save() {
      ...
      const complateLength = this.form.data.filter(item => item.userid && item.thumbnail).length
      ...
      saveJewelryTalent(obj).then(data => {
        this.$notify({
          title: '成功',
          message: '操作成功',
          type: 'success',
