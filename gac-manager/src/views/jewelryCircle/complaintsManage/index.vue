    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span @click="$router.push({ name: 'jccontentManageDetail', query: {id: row.contentId, isDraft: 0, from: 1, isWeMedia: true}})">{{row.title}}</span>
      ...
      >
        <template slot-scope="{ row }">
          <span @click="$router.push({ name: 'jcreportDetail', query: {id: row.reportId, type: row.appealStatus == 0? 1:2 , contentId: row.contentId, title: row.title, reportedType: reportTypeMap[row.reportedType], reportStatus: row.reportStatus, appealStatus: row.appealStatus}})">{{reportTypeMap[row.reportedType]}}</span>
        </template>
      </el-table-column>
      <el-table-column
        width="80px"
      ...
      >
        <template slot-scope="{ row }">
          <el-button
            ...
            @click="$router.push({name: 'jcaddComplaint', query: {id: row.reportId,contentId: row.contentId, title: row.title}})"
          >立即申诉</el-button>
        </template>
      </el-table-column>
    </el-table>
