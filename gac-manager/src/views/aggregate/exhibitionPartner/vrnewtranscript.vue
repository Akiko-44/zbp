...
<script type="text/javascript">
	...
	import { getRegion ,getRegionById, getBrandList,addEx, getIndustryPatentBanner,updateExhibitionVr,getExhibitionVr} from '@/api/aggregate/exhibition'
	...
	export default {
		...
		methods:{
			save(formName){
				this.$refs[formName].validate((valid) => {
		          if (valid) {
		            ...
		            updateExhibitionVr(this.form).then(res => {
		            	if(res.success){
		            		this.$router.push('/exhibition/vrlookPartnerList')
		            	}
		            })
		          } else {
		            return false;
			...
			getBrandList(){
				...
				getExhibitionVr(this.$route.query.id).then(res => {
					if(res.success){
						console.log(res)
						this.form = res.data
					}
