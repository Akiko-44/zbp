        ...
        <el-checkbox :key="prop.id" v-for="prop in props" :label="prop.id">
          {{prop.name}}
			...
			<el-form-item label="缩略图" prop="thumbnail">
				...
				<ImageUpload v-show="!form.thumbnail" :multiple="false" prefix="news" @successCBK="uploadSuccess" />
				<span style="color: #FF0000;">数量：1张，规格：仅支持jpg、jpeg、png格式，大小：不超过2m</span>
			</el-form-item>

			<el-form-item label="所属栏目" prop="columnId">
				<el-cascader :options="categories" v-model="selectedCategory" :change-on-select="true" @change="handleCategoryChange" :props="{value: 'id', label: 'name'}" />
			</el-form-item>
...
<script>
	...
	import { getRegular } from '@/api/news/comment'
	...
	export default {
		...
		methods: {
			save(id) {
				const set = this.$refs
				this.form.publicStatus = id // 判断发布还是存稿
