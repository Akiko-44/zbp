...
<div class="app-container calendar-list-container">
  <div class="filter-container">
    <el-input @keyup.enter.native="handleFilter" style="width: 200px" class="filter-item" placeholder="类型名称" v-model="listQuery.name">
    </el-input>
    <el-button class="filter-item" type="primary" v-waves icon="el-icon-search" @click="handleFilter">搜索</el-button>
    <el-button class="filter-item" v-if="groupTypeManager_btn_add" style="margin-left: 10px" @click="handleCreate" type="primary"
  ...
  <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
    ...
    <el-table-column align="center" label="操作" width="150" fixed="right">
      <template slot-scope="{ row, $index }">
        <el-button v-if="groupTypeManager_btn_edit" size="small" type="success" @click="handleUpdate(row, $index)">
          编辑
        </el-button>
        <el-button v-if="groupTypeManager_btn_del" size="small" type="danger" @click="handleDelete(row)">删除
        </el-button>
      </template>
    </el-table-column>
  </el-table>
      ...
      set[formName].validate(valid => {
        if (valid) {
          ...
          putObj(this.form).then(() => {
            this.dialogFormVisible = false
            // 本地更新数据，不请求服务端
            this.list[this.rowIndex].code = this.form.code
            this.list[this.rowIndex].name = this.form.name
            this.list[this.rowIndex].description = this.form.description
            this.$notify({
              title: '成功',
              message: '创建成功',
              type: 'success',
