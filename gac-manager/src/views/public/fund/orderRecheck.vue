...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      if (this.auditForm.state === 4 && this.auditForm.auditLog == undefined) {
        this.$message({
          message: '请输入不通过原因',
          type: 'warning'
        })
      ...
      const idArrs = this.auditForm.id.split(',')
      audit(this.auditForm).then(data => {
        ...
        for (let i = 0; i < this.list.length; i++) {
          if (idArrs.indexOf(this.list[i].id) !== -1) {
