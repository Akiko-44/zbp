      ...
      <el-form :model="attributeForm"
        ...
        <el-form-item label="属性值:"
          ...
          <el-input style="width: 220px;"
                    ...
                    v-model="attributeForm.subsidiaryName"> </el-input>
        </el-form-item>
...
<script>
import { subsidiarySaveOrUpdate } from '@/api/public/system'
...
export default {
  ...
  created() {
    if (this.rowData) {
      ...
      this.attributeForm = {
        id: data.id,
        categoryAttributesId: data.categoryAttributesId,
        subsidiaryName: data.subsidiaryName,
        sortOrder: data.sortOrder
      }
    } else if(this.id){
      this.attributeForm.categoryAttributesId = this.id
    }
  },
  methods: {
    cancel() {
    ...
    add() {
      this.loading = true
      subsidiarySaveOrUpdate(this.attributeForm).then(data => {
        this.$notify({
          title: '成功',
          message: '提交成功',
          type: 'success',
        ...
        this.$router.push({
          name: 'attrValueList',
          query: {id: this.attributeForm.categoryAttributesId}
        })
        this.loading = false
      }).catch(() => {
        this.loading = false
