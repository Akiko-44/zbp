    ...
    <div class="filter-container">
      ...
      >
        ...
        <el-button
          type="primary"
          v-waves
          @click="$router.push({ name:'addSecondCategory', query: {id: parentId} })"
        >添加</el-button>
        <el-button
          type="primary"
          v-waves
          @click="$router.push({ name:'menuCategory' })"
        >返回</el-button>
      </el-form>
    </div>
    ...
    >
      ...
      >
        <template slot-scope="{ row, $index }">
          <el-button
            size="small"
            @click="$router.push({ name: 'thirdCategoryList', query: {id: row.id,firstId: parentId}})"
          >三级菜单管理</el-button>
          <el-button
            size="small"
            @click="$router.push({ name: 'secondCategoryAttrList', query: {id: row.id,firstId: parentId,hasBindGoods: row.hasBindGoods}})"
          >关联类目属性</el-button>
          <el-button
            size="small"
            type="primary"
            v-if="row.hasBindGoods === 1"
            @click="$router.push({ name: 'addSecondCategory', query: {data: JSON.stringify(row)}})"
          >修改</el-button>
          <!--类目属性-->
          <el-button
            size="small"
            ...
            @click="$router.push({ name: 'addSecondCategory', query: {data: JSON.stringify(row), isEdit: 1}})"
          >修改</el-button>
          <el-button
            size="small"
            type="warning"
