<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column width="100px" align="center" label="作品ID">
        <template slot-scope="{ row }">
          <span>{{ row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column width="250px" align="center" label="作品名称">
        <template slot-scope="{ row }">
          <span
            class="table-link"
            @click="
      ...
      <el-table-column width="120px" align="center" label="作品图片">
        <template slot-scope="{ row }">
      ...
      <el-table-column width="100px" align="center" label="收藏数量">
        <template slot-scope="{ row }">
          <span>{{ row.collectNumber || "0" }}</span>
        </template>
      </el-table-column>
      <el-table-column width="100px" align="center" label="浏览数量">
        <template slot-scope="{ row }">
      ...
      <el-table-column width="200px" align="left" label="操作">
        <template slot-scope="{ row, $index }">
          <!-- <el-button
            size="small"
            @click="
              $router.push({ name: 'workDetail', query: { id: row.id } })
            "
            >查看</el-button
          > -->
          <el-button
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        id: [
          { pattern: /^[0-9]*$/, message: '作品ID需为纯数字', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      designWork(this.listQuery).then(data => {
