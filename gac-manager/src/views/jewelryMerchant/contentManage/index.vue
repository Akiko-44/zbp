    ...
    <div class="filter-container">
      ...
      >
        ...
        <el-button
          ...
          type="primary"
          v-waves
          @click="$router.push({ name:'jwcontentManageModify'})"
        >发布内容</el-button>
      </el-form>
    </div>
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span @click="$router.push({ name: 'jwcontentManageDetail', query: {id: row.id, isDraft: 0, from: 1, isWeMedia: true, contentSourceType: row.contentSourceType}})">{{row.title}}
            <img
      ...
      >
        <template slot-scope="{ row, $index }">
          ...
          <el-button
            ...
            size="small"
            type="primary"
            @click="$router.push({ name:'jwcontentManageModify',query:{id:row.id, isDraft: 0}})"
...
<script>
import { jewelryContentList, delJewelryContent, getTopicList, jewelryColumnList } from '@/api/public/jewelryRing'
