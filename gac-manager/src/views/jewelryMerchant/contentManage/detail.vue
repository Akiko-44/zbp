    ...
    >
      ...
      <el-form-item>
        <el-button
          ...
          v-if="contentForm.contentStatus == 1 && contentForm.pushStatus === 0 && from"
        >申请推送</el-button>
        <el-button
          class="auditBtn"
          size="small"
          type="primary"
          @click="$router.push({ name:'jwcontentManageModify',query:{id:contentForm.id, isDraft: 0}})"
          v-if="contentForm.contentStatus !== 0 && from && contentSourceType == 1"
        >编辑</el-button>
        <el-button
          type="primary"
          @click="preView"
...
export default {
  ...
  data() {
    return {
      ...
      contentSourceType: this.$route.query.contentSourceType,
      typeList: [
        {
          id: 1,
          menu: '图文'
  ...
  created() {
    if (this.id) {
      jewelryContentList({ id: this.id, userType: 2 }, this.isDraft).then(data => {
  ...
  methods: {
    ...
    pushSuccess(form) {
      this.$router.push({ name: 'jwcontentManage' })
    },
    handlePush(row) {
      this.push(row)
    },
