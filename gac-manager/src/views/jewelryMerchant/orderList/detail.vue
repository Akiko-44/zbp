  ...
  <div class="detail">
    ...
    <el-tabs type="border-card">
      <el-tab-pane label="订单信息">
        ...
        <h4 v-if="detail.orderInvoiceVO && detail.orderInvoiceVO.invoiceType != 0">发票信息</h4>
        <el-form
          label-position="left"
          inline
          class="table-expand"
          v-if="detail.orderInvoiceVO && detail.orderInvoiceVO.invoiceType != 0"
        >
        ...
        <h4 v-if="detail.state != 4 && expressData.data">
          物流信息
        ...
        <el-form
          ...
          inline
          class="table-expand"
          v-if="detail.state != 4 && expressData.data"
        >
        ...
        >
          <div v-if="status">物流公司：{{ detail.orderExpress.expressCompany }}</div>
          <div v-if="status">物流单号：{{ detail.orderExpress.expressNum }}</div>
          <el-table
        ...
        <el-dialog
          title="修改物流单号"
          :visible.sync="dialogExpressNum"
        >
          <el-form
            :model="expressNumForm"
            :rules="rules"
            ref="expressNumForm"
            label-position="left"
          >
            ...
            >
              <el-select
                v-model="expressNumForm.expressCompany"
                placeholder="请选择"
              >
                <el-option
            ...
            >
              <el-input
                v-model="expressNumForm.expressNum"
                placeholder="请输入新的物流单号"
              ></el-input>
            </el-form-item>
            <el-form-item label="  ">
...
export default {
  ...
  async beforeRouteEnter(to, from, next) {
    ...
    next(vm => {
      ...
      vm.detail.orderGoodVOList.map(item => {
        if (item.certificateCode) {
          vm.certificateCodeShow = true
        }
      })
    })
  },
