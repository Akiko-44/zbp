            ...
            <van-cell-group>
              <template v-for="(item, index) in data">
                <div v-if="item.children && item.children.length"
                     :key="index">
                  <h4 class="title">{{item[props.label]}}</h4>
                  <van-cell v-for="(child, i) in item.children"
                            clickable
                            :key="i"
                            :title="child[props.label]"
                            @click="currentValue = child[props.value]">
                    <van-radio :name="child[props.value]" />
                  </van-cell>
                </div>
                <div :key="index"
                     v-else>
                  <van-cell clickable
                            ...
                            @click="currentValue = item[props.value]">
                    <van-radio :name="item[props.value]" />
                  </van-cell>
                </div>
              </template>
            </van-cell-group>
