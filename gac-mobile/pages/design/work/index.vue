    ...
    <template v-if="work.caseName">
      <div class="detail-info">
        ...
        <div class="detail-row row-between gray van-hairline--bottom">
          <span>设计师: {{work.designerName}}</span>
        </div>
        <goodSku
      ...
      <div
        ...
        @click="linkTo(work.designerId)"
      >
        <img
          class="merchant-logo"
          :src="setImg(work.shopScore.logo, { w: 44 })"
        />
        ...
        <div class="rate">
          <span>商品<span
              class="red"
              v-if="designerInfo.dcpList"
            >{{designerInfo.dcpList.length}}</span></span>
          <span>签单<span
              class="red"
              v-if="designerInfo.merVO"
            >{{designerInfo.orderNumber || 0}}</span></span>
          <span>评论<span
              class="red"
              v-if="designerInfo.merVO"
            >{{designerInfo.commentNumber || 0}}</span></span>
        </div>
      </div>
  ...
  beforeMount() {
    ...
    this.$loading(this.$service('commonDmWorkInfo', { resources: [this.$route.query.id] }))
      .then(result => {
        ...
        this.$loading(this.$service('designerInfo', { resources: [this.work.designerId] }))
          .then(result => {
    ...
    async success(detail) {
      ...
      for (var i = 0; i < this.comments.length; i++) {
        for (var j = 0; j < this.comments[i].gallerys.length; j++) {
          if (this.comments[i].gallerys[j].indexOf('http://') == -1) {
            this.comments[i].gallerys[j] = 'http://image.gacjc.com/' + this.comments[i].gallerys[j]
          }
        }
      }
    },
