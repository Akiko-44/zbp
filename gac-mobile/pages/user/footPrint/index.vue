    ...
    >
      ...
      >
        <div v-if="hasLoad && goodsList.length">
          ...
          >
            <p class="edit-box">
              <span>你浏览了 <span class="red">{{ total }}</span> 件商品</span>
              <span
                class="edit-btn"
          ...
          >
            ...
            >
              <AppCard
                :data="{
                  id: goods.goodsId,
                  collectSource: goods.goodsSource,
	                imgUrl: goods.goodsPic,
	                merchantPic: goods.merchantPic,
                ...
                @click.native="toDetail(goods.goodsId, 1, (!goods.isOnSale && !goods.isDelete))"
              />
            </div>
          </div>
        </div>
      ...
      >
        <div v-if="hasLoad && circleGoodsList.length">
          ...
          >
            <p class="edit-box">
              <span>你浏览了 <span class="red">{{ circleTotal }}</span> 条珠宝圈内容</span>
              <span
                class="edit-btn"
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$loading(this.$service('userFootprint', { data: this.query })).then(result => {
    ...
    getCircleList() {
      this.dataLoading = true
      this.$loading(this.$service('circleFootprint', { data: this.query })).then(result => {
        this.dataLoading = false
        this.hasLoad = true
