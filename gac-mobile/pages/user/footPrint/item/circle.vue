    ...
    <div class="card-main flex-row">
      ...
      <div class="card-info">
        ...
        <p class="card-else">
          ...
          <span>
            <img
              src="../../../../assets/images/icon/icon_like20.png"
              width="10"
            > {{ data.likeNumber}}</span>
        </p>
      </div>
      <van-button
        class="btn collect-btn"
        color="#999999"
        @click.native.stop="deletecircleFootprint()"
        plain
      >删除</van-button>
...
export default {
  ...
  methods: {
    setImg,
    deletecircleFootprint() {
      this.$service("deleteUserFootprint", {
        data: { goodsIds: this.data.id, goodsSource: 8 }
      }).then(result => {
