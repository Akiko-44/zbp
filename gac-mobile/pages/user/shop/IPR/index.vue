<template>
	<AppView title="知识产权保护" :clickLeft="clickLeft">
		<AppList 
			:postData="postData" 
			...
			<template slot-scope="{ list }">
				<van-row gutter="10"
		         ...
		         <div class="item7">
					    ...
					    <van-row gutter="10" class="imglist"
					       @click="linkTo(item.id)">
					    	<van-col span="8" v-for="(item, i) in item.pics" :key="i" v-if="i < 3">
					    		<img :src="item"/>
					    	</van-col>
					    </van-row>
					    <div class="detail">
		...
		<div class="btn-wrap">
			<van-button @click="$router.push({name: 'user-shop-IPR-add'})"
	                block
	                class="primary-btn"
	                type="primary">
	      添加
