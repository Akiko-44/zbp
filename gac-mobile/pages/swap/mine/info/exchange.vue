<template>
<AppView class="InfoExchange">
	<AppHeader title="互换信息管理"/>
  <van-tabs
    sticky
    @click="click"
  >
    <van-tab v-for="(tab, index) in tabs" :title="tab.name" :key="index">
      <div class="tab-content">
        <AppList
          ...
          :query="{ offset: 1, limit: 20, goodsStatus: tab.state, swapble: 1 }"
          :ref="'list' + index"
        >
          <template slot-scope="{ list }">
            <InfoItem
              ...
              :data="{
                ...
                important: '',
                to: {name: 'swap-release-general', query: {id: item.id, type: 1}},
  ...
  <!--<van-button round type="danger" style="position: fixed;bottom: 5px;left: 50%;margin-left: -30px;"
  	@click="$router.push({ name: 'swap-release-general',query: { type:1 } })">添加</van-button>-->
</AppView>
</template>
