<template>
  <AppView class="confirm-order" title="选择支付方式">
    <van-radio-group v-model="payChannel">
      <van-cell-group>
      	<van-cell title="微信支付" clickable v-if="isFromWeiXinFlag">
          <i class="ico-wechat" slot="icon"></i>
          <van-radio slot="right-icon" name="20" />
        </van-cell>
        <van-cell title="支付宝支付" clickable v-else>
          <i class="ico-alipay" slot="icon"></i>
          <van-radio slot="right-icon" name="10" />
        </van-cell>
      </van-cell-group>
    </van-radio-group>
    <div style="padding: 30px 10px 0;">
      <van-button type="primary" size="small" @click="gotopay" block>立即支付</van-button>
    </div>
  </AppView>
</template>
