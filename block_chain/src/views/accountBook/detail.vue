  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="区块链平台">
        <template slot-scope="{ row }">
          <span>{{ row.blockchain.blockchainName }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="交易费用(eth)">
        <template slot-scope="{ row }">
      ...
      <el-table-column align="center" label="上链时间">
        <template slot-scope="{ row }">
...
<script>
import { accountBookList } from "@/api/accountBook";
  ...
  methods: {
    ...
    getList() {
      ...
      this.listParams.endTime = this.createTime
        ? this.createTime[1]
        : undefined;
      accountBookList(this.listParams).then(data => {
